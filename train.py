...
# cofig SAC
SAC = sac.SAC(obs_dim=PARAMs["obs_dim"], act_dim=PARAMs["act_dim"], act_limit=PARAMs["act_limit"], rnn_layer=PARAMs["rnn_layer"], bidir=PARAMs["bidir"],
...
replay_buffer = sac.core.ReplayBufferLite(
    obs_dim=PARAMs["obs_dim"], act_dim=PARAMs["act_dim"], max_obs=PARAMs["max_obs"], max_size=PARAMs["replay_size"])
...
def preNNinput(NNinput: tuple, max_obs: int, device):
    ...
    for Nth in range(NNinput[0].__len__()):
        if NNinput[1][Nth].__len__() == 0:
            NNinput[1][Nth].append([2 * SMLT.dmax] * 10)
        total_len = NNinput[1][Nth].__len__()
        idxs = list(range(total_len))
        idxs.sort(key=lambda i: norm(NNinput[1][Nth][i][6:8]), reverse=True)
